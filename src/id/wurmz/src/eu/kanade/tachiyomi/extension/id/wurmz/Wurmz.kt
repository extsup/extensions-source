    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        val details = bodyString.extractNextJsRsc<MangaDetailsDto> {
            ...
            it is kotlinx.serialization.json.JsonArray &&
                (it.getOrNull(3) as? JsonObject)?.get("dangerouslySetInnerHTML")
                    ?.let { d -> (d as? JsonObject)?.get("__html")?.jsonPrimitive?.content?.contains("ComicSeries") } == true
        }?.getOrNull(3)?.let { e -> (e as? JsonObject)?.get("dangerouslySetInnerHTML") }
            ?.let { d -> (d as? JsonObject)?.get("__html")?.jsonPrimitive?.content }
